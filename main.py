...
import utils
from lightning_modules import CaliforniaDataModule, LitModel
...
def get_args():
    ...
    parser.add_argument("--test_set", default="3")
    parser.add_argument(
        "--run_type",
        choices=["california"],
        default="california",
    )
    parser.add_argument("--train_fold", default=None)
...
def train():
    ...
    dataset_config = parser.get_configuration("dataset")
    comet_api = parser.get_configuration("comet_api")

    # Add test set
    dataset_config["key"] = args.test_set
    dataset_config["train_fold"] = args.train_fold
    ...
    if args.run_type == "california":
        datamodule = CaliforniaDataModule(**dataset_config)
    else:
        raise NotImplementedError("Not dataset exists")
    ...
    if args.ckpt_path is None:
        ...
        pl_model = LitModel.load_from_checkpoint(
            args.ckpt_path,
            identify_critical=args.identify_critical,
            identify_ignored=args.identify_ignored,
        )
